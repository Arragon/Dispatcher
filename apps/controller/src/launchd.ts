...
export const launchctl: LaunchctlExecutor = async (args) =>
  await new Promise((resolve, reject) => {
    ...
    child.on("close", (code) => (code === 0 ? resolve({ stdout, stderr }) : reject(new Error("launchctl command failed"))));
  });
...
export async function installLaunchAgent(input: {
  ...
  await (input.executor ?? launchctl)(["bootstrap", `gui/${process.getuid?.() ?? 0}`, input.plistPath]);
}
